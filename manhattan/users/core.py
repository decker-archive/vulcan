...
from ..checks import verify_email, authorize
...
async def create_user(request: Request):
    ...
    if not USERNAME_REGEX.match(name):
        raise BadData('name does not fit regex; "^[a-z0-9\-_]{3,45}$"', custom_msg='Invalid name')

    password = await loop.run_in_executor(None, bcrypt.hashpw, str(data['password']).encode(), bcrypt.gensalt(17))
    ...
    if data.get('locale'):
        if str(data['locale']) not in LOCALES:
            raise BadData('Invalid locale', str(data['locale']) + ' is not a valid and/or usable locale.')
        else:
    ...
    if data.get('screen_name'):
        ...
        if len(screen_name) > 45:
            raise BadData('Max screen_name Length is 45', custom_msg='Invalid screen_name length')
